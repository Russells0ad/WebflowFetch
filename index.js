...
const JS_REGEX = new RegExp(
  `<script\\s+src="(${JS_PATTERN})"\\s+type="text\\/javascript"\\s*><\\/script>`,
  "s"
);
const JS_REPLACE_REGEX = new RegExp(
...
async function fetchJS(url, expectedTimestamp = null) {
  ...
  const timestamp = getTimestampFromJS(js);
  checkTimestamp(timestamp, expectedTimestamp);
...
function getPages(sitemap) {
  ...
  let pages = [...sitemap.matchAll(/<loc>\s*(.*?)\s*<\/loc>/g)];
...
function getTimestampFromJS(js) {
  ...
  if (!timestampMatch) {
    console.warn("Missing JS timestamp, ignoring timestamp check");
    return null;
  }
  const timestamp = timestampMatch[1];
  return new Date(timestamp).toISOString();
